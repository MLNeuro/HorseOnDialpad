...
class HorseOnAPadTests(unittest.TestCase):
    def test_zero_length(self):
        N = DP().get_permutations(N=0, start=9)
        self.assertEqual(N, 0)

    def test_start_five(self):
        N = DP().get_permutations(N=3, start=5)
        self.assertEqual(N, 0)

    def test_start_0(self):
        N = DP().get_permutations(N=2, start=0)
        self.assertEqual(N, 2)

    def test_start_4_len_2(self):
        N = DP().get_permutations(N=2, start=4)
        self.assertEqual(N, 3)
    
    def test_start_4_len_3(self):
        N = DP().get_permutations(N=3, start=4)
        self.assertEqual(N, 5)

    def test_start_2_len_3(self):
        N = DP().get_permutations(N=3, start=2)
        self.assertEqual(N, 4)
